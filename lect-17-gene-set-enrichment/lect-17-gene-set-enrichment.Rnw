...
\begin{frame}{Objective}
  ...
  \item Pathway topology (PT) -- include pathway knowledge in
    assessing DE of genes in a set
  \end{itemize}
\end{frame}
...
\begin{frame}{Approach 5: pathway topology}
  \begin{columns}
    \column{.5\textwidth}
    \begin{itemize}
    \item Incorporate pathway topology (e.g., interactions between
      gene products) into signficance testing
    \end{itemize}
    E.g., Tarca et al., 2009, \Biocpkg{SPIA}
    \begin{itemize}
    \item Signaling Pathway Impact Analysis
    \item Combined evidence: pathway over-representation $P_{NDE}$;
      unusual signaling $P_{PERT}$ (equation 1 of Tarca et al.)
    \end{itemize}
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{our_figures/SPIA.png}
    \par {\small Evidence plot, colorectal cancer. Points: pathway
      gene sets.  Significant after Bonferroni (red) or FDR (blue)
      correction.}
  \end{columns}
\end{frame}

\begin{frame}{Approach 6: issues with sequence data?}
  \begin{columns}
    \column{.5\textwidth}
    \begin{itemize}
...
\begin{frame}{Approach 7: \emph{de novo} discovery}
  \begin{itemize}
