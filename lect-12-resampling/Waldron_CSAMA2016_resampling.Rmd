...
![Variability of 2-fold cross-validation](ISLR_Fig52.png)
...
* Example from training and test sets simulated from an identical true model
...
set.seed(2)
bootDiff = function(){
  sleepboot = sleep[sample(1:nrow(sleep), replace = TRUE), ]
  mean(sleepboot$extra[sleepboot$group==1]) - mean(sleepboot$extra[sleepboot$group==2])
}
...
* What if only one margin were profiled?
    1. take a bootstrap sample of patients
    2. randomly select a single margin from each patient
    3. calculate Hazard Ratio
    4. Repeat to estimate confidence interval or perform hypothesis test
