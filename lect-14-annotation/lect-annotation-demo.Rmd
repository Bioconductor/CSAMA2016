---
title: "Lecture 14 Demo: Annotating Genes, Genomes, and Variants"
...
output:
  BiocStyle::html_document:
    toc: true
vignette: >
  % \VignetteIndexEntry{Lecture 14 Demo: Annotating Genes, Genomes, and Variants}
...
valid <- sum(width(cdsByGene)) %% 3 == 0
table(valid)
```

Playing well with BSgenome packages
...
- use `GenomicFeatures::makeTxDbFromGFF()`

# OrganismDb packages

`OrganismDb` (e.g., `r Biocpkg("Homo.sapiens")`) packages.

- org + TxDb + GO.db packages
...
```{r biomaRt}
library(biomaRt)
head(listMarts())                               # available marts,
head(listDatasets(useMart("ensembl")))          # datasets in mart, 69!
ensembl <-                                      # fully specified mart
  useMart("ensembl", dataset = "hsapiens_gene_ensembl")

head(listFilters(ensembl), 3)                   # filters, lots!
...
query(ah, c("Ensembl", "release-83", "GTF"))
## We could get the one from e.g. Bos taurus:
DB <- ensDbFromAH(ah["AH50355"])
edb <- EnsDb(DB)
edb
```
